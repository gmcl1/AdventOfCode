...
public class PartTwo {
    ...
    public void test() {
        ...
        String[] validNums = {"one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};
        ...
        for (String line : INPUT.split("\n")) {
            ...
            for (int i = 0; i < line.length(); i++) {
                ...
                for (String num : validNums) {
                    if (b.toString().contains(num)) {
                        b = new StringBuilder(b.toString().replace(num, correspondence(num))).append(c);
                    }
                }

            }

            converted.add(b.toString().replaceAll("[aA-zZ]", ""));

        }

        int total = 0;
        for (String num : converted) {
            int number = Integer.parseInt(num.charAt(0) + "" + num.charAt(num.length() - 1));
            total += number;
        }
    ...
    public String correspondence(String num) {
        return switch (num) {
            case "one" -> "1";
            case "two" -> "2";
            case "three" -> "3";
