...
public class PartTwo {
    ...
    public void test(){
        ...
        Map<Integer, Map<String, Integer>> required = new HashMap<>();
        ...
        for(String gameOverview : INPUT.split("\n")){
            ...
            for(String cubeSet : gameCubes){
                ...
                for(String set : sets){
                    ...
                    if(required.containsKey(gameId)){

                        if(required.get(gameId).containsKey(color)) {
                            int currentAmount = required.get(gameId).get(color);
                            if (currentAmount < amount) {
                                required.get(gameId).replace(color, amount);
                            }
                        } else {
                            required.get(gameId).put(color, amount);
                        }


                    } else {
                        required.put(gameId, new HashMap<>(){{
                            put(color, amount);}
                        });
                    }
        ...
        AtomicInteger powerTotal = new AtomicInteger();

        required.forEach((game, data) -> {
            int gTotal = 1;
            List<Integer> nums = new ArrayList<>(data.values());
            for (Integer num : nums) {
                gTotal = gTotal * num;
            }
            powerTotal.addAndGet(gTotal);
        });
